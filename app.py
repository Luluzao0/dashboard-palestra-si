...
c2.metric("⭐ Nota Média",f"{df.iloc[:,-1].astype(float).mean():.2f}/10")
c3.metric("👥 Taxa >8",f"{(df.iloc[:,-1].astype(float)>=8).mean()*100:.0f}%")
c4.metric("📅 Período","14-15 Out")
c5.metric("✅ Completos",f"{df.iloc[:,-1].notna().sum()}")
...
with tab1:
 ...
 with col1:
  ...
  notas=df.iloc[:,-1].astype(float)
  notas.value_counts().sort_index().plot(kind='bar',ax=ax,color='#667eea',edgecolor='black',alpha=0.7)
 ...
 with col1:
  ...
  st.pyplot(fig)
 with col2:
  media_por_idade=df.groupby('Idade')[df.columns[-1]].agg(['mean','count','std'])
...
with tab2:
 ...
 dados_temas=[]
 for nome,col in temas:
  if col in df.columns:
   sim=len(df[df[col].astype(str).str.contains('Sim',case=False,na=False)])
   nao=len(df[df[col].astype(str).str.contains('Não',case=False,na=False)])
   total=sim+nao
 ...
 bars=ax.barh(df_temas['Tema'],df_temas['Taxa %'],color=colors,edgecolor='black',alpha=0.8)
...
with tab3:
 ...
 with col2:
  if pesq_col in df.columns:
   ...
   st.write(f"  - Nota média: {com_pesq.iloc[:,-1].astype(float).mean():.2f}")
   st.write(f"**Sem experiência:** {len(sem_pesq)} pessoas")
   st.write(f"  - Nota média: {sem_pesq.iloc[:,-1].astype(float).mean():.2f}")
   if len(com_pesq)>1 and len(sem_pesq)>1:
    t_stat,p_valor=stats.ttest_ind(com_pesq.iloc[:,-1].astype(float).dropna(),sem_pesq.iloc[:,-1].astype(float).dropna())
